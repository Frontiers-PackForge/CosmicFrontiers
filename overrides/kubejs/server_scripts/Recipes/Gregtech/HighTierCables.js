ServerEvents.recipes(event => {
    ...
    new Cable('gtceu', 'vanadium_gallium', 1, event).cables()
    new Cable('gtceu', 'naquadah', 1, event).cables()
    new Cable('gtceu', 'trinium', 1, event).cables()
    //UV
    new Cable('gtceu', 'tritanium', 2, event).cables()
    new Cable('gtceu', 'naquadah_alloy', 2, event).cables()
    new Cable('gtceu', 'yttrium_barium_cuprate', 2, event).cables()
    new Cable('cosmiccore', 'nevramite', 2, event).cables()
    new Cable('cosmiccore', 'living_igniclad', 2, event).cables()
    new Cable('cosmiccore', 'sol_steel', 2, event).cables()
    new Cable('cosmiccore', 'starmetal', 2, event).cables()
    //UHV
    new Cable('cosmiccore', 'europium', 4, event).cables()
})

function Cable(modID, cable, fluidMult, event) {
    this.modID = modID;
